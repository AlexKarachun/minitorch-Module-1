...
def test_sigmoid(a: float) -> None:
    ...
    * It is strictly increasing.
    ...
    assert abs(1 - sigmoid(a) - sigmoid(-a)) < 1e-2 
    assert abs(sigmoid(0) - 0.5) < 1e-2
    assert sigmoid(a / 10) < sigmoid(a / 10 + 1)
...
def test_transitive(a: float, b: float, c: float) -> None:
    ...
    assert [1, lt(a, c)][lt(a, b) and lt(b, c)]
...
def test_sum_distribute(ls1: List[float], ls2: List[float]) -> None:
    ...
    """
    assert abs(sum(ls1) + sum(ls2) - sum(addLists(ls1, ls2))) <= 1e-2
